...
def traverse(grid, row, col, visited):
    ...
    for trial in trials:
        ...
        if is_inbound(grid, n_row, n_col) and type(grid[n_row][n_col]) == int and grid[n_row][n_col]-grid[row][col] == 1:
            out += traverse(grid, n_row, n_col, visited.copy())
...
def solver(grid):
    ...
    for start in starts:
        ...
        print(f'start {start}')
        paths = traverse(grid, start_row, start_col, set())
        print(f'paths: {paths}')
        total += paths
...
lines = transform(open('sample2_1'))
out = solver(lines)
print(out)
assert out == 3


lines = transform(open('sample2_2'))
out = solver(lines)
print(out)
assert out == 13


lines = transform(open('sample2_3'))
out = solver(lines)
print(out)
assert out == 227
