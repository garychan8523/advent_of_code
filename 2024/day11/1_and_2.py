...
def blink(num, target, iter=0, count=0):
    ...
    if num == 0:
        return blink(1, target, iter, count)
    
    digits_length = get_digits_length(num)
    if digits_length % 2 == 0:
        half_length = digits_length // 2
        left_count = blink(get_first_n_digits(num, half_length), target, iter, count)
        right_count = blink(get_last_n_digits(num, half_length), target, iter, count)
        return left_count + right_count
    else:
        return blink(num * 2024, target, iter, count)

    
    
def solver(stone, target):
    return blink(stone, target, iter=0, count=1)
